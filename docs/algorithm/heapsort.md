...
对于二叉堆的应用是在`scheduler`包中, 有 2 个数组[`taskQueue`和`timerQueue`](https://github.com/facebook/react/blob/v19.2.6/packages/scheduler/src/forks/Scheduler.js), 它们都是以`最小堆`的形式进行存储, 这样就能保证以`O(1)`的时间复杂度, 取到数组顶端的对象(优先级最高的 task).

具体的调用过程被封装到了[`SchedulerMinHeap.js`](https://github.com/facebook/react/blob/v19.2.6/packages/scheduler/src/SchedulerMinHeap.js), 其中有 2 个函数`siftUp`,`siftDown`分别对应向上调整和向下调整.
