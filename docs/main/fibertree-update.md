...
本节讨论`对比更新`这种情况. 在 v19 中, 所有应用通过`createRoot`启动后默认运行在`Concurrent`模式下, 但本节聚焦`fiber树构造`本身的算法, 与是否分片渲染无关; 行文中给出的同步流程图同样适用于`Concurrent`模式中 SyncLane / DefaultLane 等不让出主线程的批次. 在阅读本节之前, 最好对[fiber 树构造(初次创建)](./fibertree-create.md)有一些了解, 其中有很多相似逻辑不再重复叙述, 本节重点突出`对比更新`与`初次创建`的不同之处.
...
前文[reconciler 运作流程](./reconciler-workflow.md#输入)中总结的 4 个阶段(从输入到输出), 其中承接输入的函数只有`scheduleUpdateOnFiber`([源码地址](https://github.com/facebook/react/blob/v19.2.6/packages/react-reconciler/src/ReactFiberWorkLoop.js)).在`react-reconciler`对外暴露的 api 函数中, 只要涉及到需要改变 fiber 的操作(无论是`首次渲染`或`对比更新`), 最后都会间接调用`scheduleUpdateOnFiber`, `scheduleUpdateOnFiber`函数是输入链路中的`必经之路`.
...
在`Component`对象的原型上挂载有`setState`([源码链接](https://github.com/facebook/react/blob/v19.2.6/packages/react/src/ReactBaseClasses.js)):
...
在[fiber 树构造(初次创建)](./fibertree-create.md)中的`beginWork`阶段, class 类型的组件初始化完成之后, `this.updater`对象如下([源码链接](https://github.com/facebook/react/blob/v19.2.6/packages/react-reconciler/src/ReactFiberClassComponent.js)):

```js
// v19 简化版 (eventTime 已删除)
const classComponentUpdater = {
  ...
  enqueueSetState(inst, payload, callback) {
    ...
    const update = createUpdate(lane); // v19: 不再接收 eventTime
    update.payload = payload;
    if (callback !== undefined && callback !== null) {
      update.callback = callback;
    }
    // 3. 将update对象加入 concurrentQueues 待提交队列, 返回 root
    const root = enqueueUpdate(fiber, update, lane);
    if (root !== null) {
      // 4. 进入reconciler运作流程中的`输入`环节
      scheduleUpdateOnFiber(root, fiber, lane);
      entangleTransitions(root, fiber, lane); // 配合 useTransition
    }
  },
};
```
...
在`function类型`组件中, 如果使用`hook(useState)`, 则可以通过`hook api`暴露出的`dispatchSetState`/`dispatchReducerAction`([源码链接](https://github.com/facebook/react/blob/v19.2.6/packages/react-reconciler/src/ReactFiberHooks.js))来更新. 注意 v18 起原来的`dispatchAction`已被拆为这两个具名函数, 分别对应`useState`和`useReducer`

```js
// v19 简化版 dispatchSetState (useState 专用; useReducer 走 dispatchReducerAction, 类似但不带 eager 优化)
function dispatchSetState<S, A>(
  fiber: Fiber,
  queue: UpdateQueue<S, A>,
  action: A,
) {
  // 1. 计算 lane (v18+ 不再传 eventTime)
  const lane = requestUpdateLane(fiber);
  // 2. 创建 update 对象 (v18+ 字段: revertLane / hasEagerState)
  ...
  // 3. (eagerState 优化, 命中则提前 return)
  // 4. 入队到 concurrentQueues, 拿到 root
...
const root = createRoot(document.getElementById('root')); // v18+ 只创建一次
function tick() {
  const element = (
    <div>
      <h1>Hello, world!</h1>
  ...
  root.render(element); // v18+ 复用同一个 root
}
...
逻辑来到[scheduleUpdateOnFiber](https://github.com/facebook/react/blob/v19.2.6/packages/react-reconciler/src/ReactFiberWorkLoop.js)函数:
...
export function scheduleUpdateOnFiber(
  ...
  // 标记 root 上 lane (内部完成 v17 时 markUpdateLaneFromFiberToRoot 的工作)
  markRootUpdated(root, lane);
  // 进入第2阶段: 同步走 microtask, 并发走 Scheduler
...
1. **沿父链冒泡 lane**: v17 中通过`markUpdateLaneFromFiberToRoot(fiber, lane)`一个函数完成: 既要沿父链冒泡设置`fiber.lanes / childLanes`, 又要返回`root`. v18 起这两步被拆分为:
   - `getRootForUpdatedFiber(sourceFiber)`: 仅负责沿父链上溯获取`root`, 并把过程中的`fiber.lanes / childLanes`设置到 alternate 双链上.
   - `markRootUpdated(root, lane)`: 仅负责在`root`层面登记新的 lane, 加入待调度队列.
   - 上面这两步的具体调用发生在`enqueueUpdate` / `enqueueConcurrentRenderForLane`内部.

```js
// v19 ReactFiberConcurrentUpdates.js (简化版)
function getRootForUpdatedFiber(sourceFiber: Fiber): FiberRoot | null {
  // 在 sourceFiber 上标记 lane, 同时沿 return 指针向上把 childLanes 标记到所有祖先
  // 同时 alternate 双链也一并标记
...
// v19 ReactFiberLane.js
export function markRootUpdated(root: FiberRoot, updateLane: Lane) {
  root.pendingLanes |= updateLane;
  // 如果不是 idle, 清掉 suspended/pinged
...
下图表示了 v17 时期 `markUpdateLaneFromFiberToRoot` 的具体作用, 在 v19 中它被拆分为`getRootForUpdatedFiber + markRootUpdated`, 但效果一致:
...
2. `对比更新`不再像 v17 legacy 那样有"直接调用`performSyncWorkOnRoot`"的快捷路径, 全部通过`ensureRootIsScheduled`走 microtask. 同步车道在 microtask 末尾冲刷, 调用链最终仍为`performSyncWorkOnRoot --> renderRootSync --> workLoopSync`, 与`初次构造`一致.

在[renderRootSync](https://github.com/facebook/react/blob/v19.2.6/packages/react-reconciler/src/ReactFiberWorkLoop.js)中:
...
`beginWork(current, unitOfWork, subtreeRenderLanes)`([源码地址](https://github.com/facebook/react/blob/v19.2.6/packages/react-reconciler/src/ReactFiberBeginWork.js)).
...
1. `!includesSomeLane(renderLanes, updateLanes)`这个判断分支, 包含了`渲染优先级`和`update优先级`的比较(详情可以回顾[fiber 树构造(基础准备)](./fibertree-prepare.md#lanes)中`优先级`相关解读), 如果当前节点无需更新, 则会进入`bailout`逻辑.
...
2. 如果是需要删除的`fiber`, v18 起处理方式有变更: [不再单独打`Deletion`标志位并加入旧的`effectList`](https://github.com/facebook/react/blob/v19.2.6/packages/react-reconciler/src/ReactChildFiber.js), 而是把被删除的子 fiber 放入父节点的`returnFiber.deletions: Fiber[]`数组, 同时在父节点上打上`ChildDeletion`标志位. commit 阶段会基于`subtreeFlags & ChildDeletion`从根 DFS 时, 在父节点上统一处理`deletions`数组中的所有节点(详见[fiber 树渲染](./fibertree-commit.md)).

### 回溯阶段 completeWork

`completeUnitOfWork(unitOfWork)函数`([源码地址](https://github.com/facebook/react/blob/v19.2.6/packages/react-reconciler/src/ReactFiberWorkLoop.js))在`初次创建`和`对比更新`逻辑一致, 都是处理`beginWork` 阶段已经创建出来的 `fiber` 节点, 最后创建(更新)DOM 对象, 并沿父链冒泡`subtreeFlags`(v18 起取代了 v17 的"上移副作用队列").
...
```js
// v19 简化版 (prepareUpdate 已删除, 仅做轻量 props 比较, 决定是否打 Update 标记)
updateHostComponent = function (
  ...
  // v19: 不再计算 updatePayload, 直接打 Update 标记
...
可以看到在更新过程中, 如果 DOM 属性有变化, 不会再次新建 DOM 对象, 而是设置`fiber.flags |= Update`, 等待`commit`阶段处理([源码链接](https://github.com/facebook/react/blob/v19.2.6/packages/react-reconciler/src/ReactFiberCompleteWork.js)).

> v19 重要变化: complete 阶段**不再调用** `prepareUpdate` 计算 props diff (见[#26583](https://github.com/facebook/react/pull/26583)). v17 时期 `completeWork` 会比较 `oldProps` 与 `newProps` 得到 `updatePayload` 数组放到 `workInProgress.updateQueue` 上, commit 时按数组应用; v19 中干脆把这一步推迟到 `commitMutationEffects` 内统一比较, 既减少了 render 阶段的工作量(被丢弃的渲染就不用浪费 diff), 也降低了内存占用.
...
- `beginWork`执行过程: 调用`updateHostComponent`
  - 本示例中`button`的子节点是一个[直接文本节点](https://github.com/facebook/react/blob/v19.2.6/packages/react-dom-bindings/src/client/ReactFiberConfigDOM.js),设置[nextChildren = null](https://github.com/facebook/react/blob/v19.2.6/packages/react-reconciler/src/ReactFiberBeginWork.js)(源码注释的解释是不用在开辟内存去创建一个文本节点, 同时还能减少向下遍历).
